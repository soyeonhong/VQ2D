...
class Task:
    def __init__(self, config, annots, output_dir, args):
        super().__init__()
        self.config = config
        self.annots = annots
    ...
    def run(self, model, config, device):
        ...
        clip_reader = decord.VideoReader(clip_path, num_threads=8)
...
def inference_video(config, model, clip_path, clip_reader, query_frame, visual_crop, save_path, device, start_frame, end_frame):
    '''
