...
class Task:
    def __init__(self, config, annots, pred_dir):
        super().__init__()
        self.config = config
        self.annots = annots
    ...
    def run(self, config, device):
        ...
        for key, annot in zip(self.keys, self.annots):
            ...
            if 'end_frame' in list(annot.keys()):
                query_frame = annot["end_frame"] - annot['start_frame'] + 1
            else:
                query_frame = annot["query_frame"]
            visual_crop = annot["visual_crop"]
            save_path = os.path.join(self.pred_dir, f'{annot_key}.pt')
            
            if not os.path.exists(save_path):
                print("Cache file not found: ", save_path)
                continue
